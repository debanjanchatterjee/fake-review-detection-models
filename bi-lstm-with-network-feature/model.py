...
class bilstm(nn.Module):
  '''LSTM'''
  def __init__(self,dict_sz,hid_sz,device,num_layer = 2,inp_sz = 300):
    ...
    self.linearf = nn.Linear(14,50)
    self.linear1 = nn.Linear(self.hid_sz*2*4,self.hid_sz)
    self.linear2 = nn.Linear(self.hid_sz+50,self.hid_sz)
  ...
  def forward(self, head, body,features):
    ...
    output = torch.cat((head_hid,body_hid,torch.abs(head_hid-body_hid),head_hid*body_hid),dim=1)
