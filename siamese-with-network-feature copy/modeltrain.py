...
class main_model(nn.Module):
  def __init__(self,hid_sz,device):
    super(main_model, self).__init__()

    self.hid_sz = hid_sz
    self.device = device
  
    self.bilstm_model = siamese(hid_sz,device)
  ...
  def train(self,head,body,label,n_epochs,reload,batch_size = 128):
    ...
    n_batches = int(len(head)/batch_size)
    ...
    for i in range(start_epoch,n_epochs):
      ...
      time_expected = int((time_escaped/i+1)*(n_epochs - start_epoch))
