...
class main_model(nn.Module):
  ...
  def train(self,head_i,body_i,features,label,n_epochs,reload,batch_size = 128):
    # initalizing Adam optimizer and CrossEntropyLoss(Negitive log liklyhood loss)
    ...
    for i in range(start_epoch,n_epochs):
      ...
      for b in range(n_batches):
        ...
        outputs = self.siamese_model(head_i_batch,body_i_batch,features_batch)
  ...
  def predict(self,head_i,body_i,features):
    ...
    output = self.siamese_model.forward(head_i, body_i, features)
    out = []
