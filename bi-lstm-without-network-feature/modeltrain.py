...
class main_model(nn.Module):
  ...
  def train(self,head,body,label,n_epochs,reload,batch_size = 128):
    ...
    if reload:
      ...
      optimizer, start_epoch, batch_id = self.load_ckp(self.path+"checkpoint.pt", optimizer)
    ...
    for i in range(start_epoch,n_epochs):
      ...
      for b in range(start_batch,n_batches):
        ...
        if (b+1 == n_batches):
          ...
          self.save_ckp(i+1,b,optimizer,self.path+'checkpoint.pt')
        ...
        if (b+1)%(int((n_batches-start_batch)/5)) == 0:
          ...
          self.save_ckp(i,b,optimizer,self.path+'checkpoint.pt')
