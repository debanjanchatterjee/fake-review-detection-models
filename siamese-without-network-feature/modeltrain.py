...
class main_model(nn.Module):
  def __init__(self,dict_sz,hid_sz,device,path):
    ...
    self.siamese_model = siamese(dict_sz,hid_sz,device)

  def train(self,head_i,body_i,label,n_epochs,reload,batch_size = 128):
    # initalizing Adam optimizer and CrossEntropyLoss
    ...
    if reload:
      ...
      optimizer, start_epoch = self.load_ckp(self.path+"checkpoint.pt", optimizer)
    ...
    for i in range(start_epoch,n_epochs):
      ...
      for b in range(n_batches):
        ...
        outputs = self.siamese_model(head_i_batch,body_i_batch)
      ...
      self.save_ckp(i,optimizer,self.path+'checkpoint.pt')
  ...
  def save_loss(self,epoch,loss):
    ...
    with open(self.path+'loss.csv', 'a') as file:
      file.write("{},{}\n".format(epoch,loss))
      file.close()
  ...
  def save_ckp(self,epoch,optimizer,checkpoint_dir):
    ...
    checkpoint = {
        'epoch': epoch + 1,
        'bilstm_state_dict': self.siamese_model.state_dict(),
  ...
  def load_ckp(self,checkpoint_fpath, optimizer):
    ...
    self.siamese_model.load_state_dict(checkpoint['bilstm_state_dict'])
    #optimizer.load_state_dict(checkpoint['optimizer'])
    return ( optimizer, checkpoint['epoch'])

  def predict(self,head_i,body_i):
    batch_size = head_i.shape[0]
    output = self.siamese_model.forward(head_i, body_i)
    out = []
    #print(output.shape)
